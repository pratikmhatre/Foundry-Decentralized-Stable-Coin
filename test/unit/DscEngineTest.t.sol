...
contract DscEngineTest is Test {
    ...
    function testInvalidCollateralToPriceFeedDataReverts() external {
        ...
        vm.expectRevert(
            DSCEngine.DSC__InvalidCollateralToPricefeedData.selector
        );
    ...
    function testDepositCollateralIncreasedUserCollateralBalance()
        external
        depositCollateral
    {
        vm.prank(USER);
        assert(
            engine.getUserCollateralByTokenAddress(wethAddress) ==
                COLLATERAL_AMOUNT
        );
    }
    ...
    function testMintDSCRevertsIfHealthFactorBreaks()
        external
        depositCollateral
    {
        vm.expectRevert();
    ...
    function testDepositCollateralAndMintDscRevertsIfDscAmountBreaksHealthFactor()
        external
    {
        vm.startPrank(USER);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                DSCEngine.DSCEngine__PoorHealthFactor.selector,
                0
            )
        );
    ...
    function testGetAccountInfo() external depositCollateralAndMintDSC {
        vm.startPrank(USER);
        (uint256 dscMinted, uint256 collateralInUsd) = engine
    ...
    function testRedeemCollateralEmitsCollateralRedeemedEvent()
        external
        depositCollateral
    {
        vm.startPrank(USER);
    ...
    function testRedeemCollateralEmitsTransferSuccessEvent()
        external
        depositCollateral
    {
        vm.startPrank(USER);
    ...
    function testBurnDSCReducesUserDSCBalance()
        external
    ...
    function testGetTokenFromUSDGivesCorrectAnswer() external {
        ...
        uint256 actualValue = engine.getTokenAmountFromUSD(
            wethAddress,
            4800 ether
        );
    ...
    //liquidate user
    function testLiquidateUserRevertsIfHealthFactorOk()
        external
        depositCollateralAndMintDSC
    {
        vm.startPrank(USER);
    ...
    function testLiquidateUserClearsUsersDSCDebt() external {
        (address user1, , ) = _createUserWithBalance(
            111,
            100000 ether,
            50000 ether
        );
        (address user2, , ) = _createUserWithBalance(
            222,
            50 ether,
            50000 ether
        );
    ...
    function testLiquidateUserReducesUsersCollateral() external {
        ...
        uint256 debtInEth = engine.getTokenAmountFromUSD(
            wethAddress,
            50000 ether
        );
    ...
    */
    ...
    function _createUserWithBalance(
        uint160 userId,
